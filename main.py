...
async def on_ready():
    print(
        f"Logged on as {bot.user} and monitoring {amiiboname}'s matches, with {amiiboname_2} coming up next!"
    )
...
async def pingmii(message):
    ...
    if (
        ...
        if ENABLEAMIIBOID is not None and RATING_OR_MATCHES == "Rating":
            ...
            if oldamiibo["rating"] <= newamiibo["rating"]:
                disablepayload = {"is_active": False}
                enablepayload = {"is_active": True}
            ...
            if newamiibo["rating"] <= oldamiibo["rating"]:
                disablepayload = {"is_active": False}
                enablepayload = {"is_active": True}
