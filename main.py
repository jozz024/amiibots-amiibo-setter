...
amiiboname = amiibo_data_name.json()['data']
...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        if message.author == client.user:
            return
        if (USERID in message.content or USER in message.content) and amiiboname['name'] in message.content and message.channel.id == SHOUTBOXID:
